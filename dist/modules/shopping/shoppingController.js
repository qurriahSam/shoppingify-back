...
const getHistory = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const items = yield shoppingModel_1.HistoryShopping.find({ current: false }).select('title status date');
        res.json(items);
    }
    catch (error) {
        next(error);
...
const getCurrentShoppingList = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const currentShoppingList = yield shoppingModel_1.HistoryShopping.findOne({ current: true }).exec();
